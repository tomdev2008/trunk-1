...
public class EaseEmojiconScrollTabBar extends RelativeLayout {
    ...
    public void selectedTo(int position) {
        ...
        for (int i = 0; i < tabList.size(); i++) {
            if (position == i) {
                tabList.get(i).setBackgroundColor(getResources().getColor(R.color.abn_yueai_text_little_gray));
            } else {
                tabList.get(i).setBackgroundColor(getResources().getColor(R.color.abn_yueai_text_white));
            }
        }
    }
