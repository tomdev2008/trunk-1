...
public class CommonPushMessageHandler {
        ...
        if (message.needLogin == 1) {
            if (autoContinue) {
                ...
                if (StringUtils.isEmpty(UserUtil.isLoginValid(true))) {
                    SplashActivity.launchIntent(context, true);
                    return true;
                }
            }
        }
    ...
    */
    public static Intent createIntent(Context context, int messageType, String targetId, String url, PushMessage message) {
        ...
        if (messageType == PushMessage.MessageType_UserDetail) {
            UserEntity userEntity = null;
            if (message != null && message.data != null && message.data.user != null) {
                userEntity = message.data.user;
            }
            intent = new Intent(context, UserDetailActivity.class);
            intent.putExtra(IntentExtraKey.INTENT_EXTRA_KEY_USERINFO, userEntity);
            intent.putExtra(IntentExtraKey.INTENT_EXTRA_KEY_UUUID, targetId);

//            UserDetailActivity.launchIntent(context, userEntity);
        } else if (messageType == PushMessage.MessageType_FansList) {
            intent = new Intent(context, FansListActivity.class);
            intent.putExtra(FansListActivity.INTENT_EXTRA_KEY_LIST_TYPE, FansListActivity.LIST_TYPE_FANS_LIST);
//            FansListActivity.launchIntentFansList(context);
        } else if (messageType == PushMessage.MessageType_BrowserList) {
            intent = new Intent(context, FansListActivity.class);
            intent.putExtra(FansListActivity.INTENT_EXTRA_KEY_LIST_TYPE, FansListActivity.LIST_TYPE_BROWSE_LIST);
//            FansListActivity.launchIntentBrowseList(context);
        }
