...
//		}
	 ...
	 * 生成区块列表，用于图片倒入
	 * @param imgList 图片列表
	 * @param perDuration 每张图片放多久
	 */
	public void buildMediaPart(ArrayList<String> imgList,int perDuration)
	{
		File f=new File(mOutputDirectory);
		if (!f.exists()) {
			f.mkdirs();
		}
		for (int i = 0; i < imgList.size(); i++) {
			...
			mCurrentPart.position=getDuration();
			mCurrentPart.index=mMediaList.size();
			mCurrentPart.thumbPath=imgPath;
			mCurrentPart.mediaPath=mOutputDirectory + File.separator + mCurrentPart.index + ".ts";
			mCurrentPart.audioPath = mOutputDirectory + File.separator + mCurrentPart.index + ".a";
			mCurrentPart.duration = perDuration;
			mCurrentPart.startTime = 0;
			mCurrentPart.endTime = perDuration;
			mCurrentPart.cutStartTime = 0;
			mCurrentPart.cutEndTime = perDuration;
			mCurrentPart.tempPath = imgPath;
			mCurrentPart.type = MEDIA_PART_TYPE_IMPORT_IMAGE;
			mMediaList.add(mCurrentPart);
			
			FFMpegUtils.convertImage2Video(mCurrentPart);
		}
	}
