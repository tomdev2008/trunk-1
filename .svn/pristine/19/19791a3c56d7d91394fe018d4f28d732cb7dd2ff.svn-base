 ...
 *
 */
public abstract class MediaRecorderBase implements Callback, PreviewCallback,
		IMediaRecorder {

	public interface OnParamsPreparedListener {
	 ...
	 * 预期的预览尺寸
	 */
	private int preferPreviewSize = 480;
	private OnParamsPreparedListener paramsListener;

	public void setParamsListener(OnParamsPreparedListener listener) {
		paramsListener = listener;
	}

	public void setPreferPreviewSize(int w) {
		preferPreviewSize = w;
	}
	...
	protected void prepareCameraParaments() {
		...
		List<Size> spSizeList = mParameters.getSupportedPreviewSizes();
		mParameters.setPreviewSize(spSizeList.get(spSizeList.size() - 1).width,
				spSizeList.get(spSizeList.size() - 1).height);// 3:2
		for (int i = spSizeList.size() - 1; i >= 0; i--) {
			Log.i("====>", spSizeList.get(i).height + "");
			if (spSizeList.get(i).height >= preferPreviewSize) {
				mParameters.setPreviewSize(spSizeList.get(i).width,
						spSizeList.get(i).height);// 3:2
				break;
			}
		}
		if (paramsListener != null) {
			paramsListener.onPrepared(mParameters.getPreviewSize().width, mParameters.getPreviewSize().height);
		}
		// 设置输出视频流尺寸，采样率
		...
		// isSupported(mParameters.getSupportedSceneModes(),
	...
	protected void setPreviewCallback() {
		...
		if (size != null) {
			...
			Log.e("Yixia",
					"startPreview...setPreviewCallbackWithBuffer...width:"
	...
	public void testPreviewFrameCallCount() {
		new CountDownTimer(1 * 60 * 1000, 1000) {
			...
			public void onTick(long millisUntilFinished) {
				Log.e("[Vitamio Recorder]", "testFrameRate..."
						+ mPreviewFrameCallCount);
	...
	protected void concatVideoParts() {
		new AsyncTask<Void, Void, Boolean>() {
			...
			protected Boolean doInBackground(Void... params) {
				// 合并ts流
				String cmd = String
						.format("ffmpeg %s -i \"%s\" -vcodec copy -acodec copy -absf aac_adtstoasc -f mp4 -movflags faststart \"%s\"",
	...
	public static class EncodeHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_ENCODE_PROGRESS:
				// 查询片段是否都已经转码完成
				final int progress = UtilityAdapter.FilterParserAction("",
						UtilityAdapter.PARSERACTION_PROGRESS);
				if (progress == 100) {
