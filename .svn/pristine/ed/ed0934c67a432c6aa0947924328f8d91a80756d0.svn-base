...
public class DialogUtil {
    ...
    public static void showDatePicker(final Activity context,
        ...
        final int MM = time.get(Calendar.MONTH)+1;
        int DD = time.get(Calendar.DATE);
        dialog.getDatePicker().init(year, monthOfYear, dayOfMonth, new DatePicker.OnDateChangedListener() {
            @Override
            public void onDateChanged(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
                if (isDateAfter(view)) {
                    view.init(YY- Constant.minShowAge, 11, 30, this);
                }
                if (isDateBefore(view)) {
                    view.init(YY-Constant.maxShowAge, 1, 1, this);
                }

            }
            private boolean isDateAfter(DatePicker tempView) {
                if (tempView.getYear() > YY-18) {
                    return true;
                } else
                    return false;
            }
    ...
    public static Dialog showSelectDialog(Activity activity, String title, String msg, String positive, String negative, OnClickListener listener) {
        ...
        try {
            ...
            if (positive != null) {
                dialog.setButton(Dialog.BUTTON_POSITIVE, positive,
                        listener);
            }
            if (negative != null) {
                dialog.setButton(Dialog.BUTTON_NEGATIVE, negative,
                        listener);
            }
    ...
    public static Dialog showDialog(Activity activity, String title,
        ...
        return showSelectDialog(activity, title, msg, needBtn ? activity.getResources()
                .getString(R.string.abn_yueai_confirm) : null, null, new OnClickListener() {
