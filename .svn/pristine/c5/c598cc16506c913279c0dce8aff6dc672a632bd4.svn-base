...
import com.yang.camera.record.MediaObject.MediaPart;
...
public class FFMpegUtils {
	...
	public static String getLogCommand() {
		if (VCamera.isLog())
			...
			return " -d \"" + VCamera.getVideoCachePath()
					+ VCamera.FFMPEG_LOG_FILENAME_TEMP + "\" -loglevel verbose";
	}
	 ...
	 */
	@SuppressLint("NewApi")
	public static boolean importVideo(MediaPart part, int mWindowWidth,
			int videoWidth, int videoHeight, int cropX, int cropY,
			boolean hasAudio) {
		if (part != null && !StringUtils.isEmpty(part.tempPath)) {
			...
			if (f != null && f.exists() && !f.isDirectory()) {
				...
				if (DeviceUtils.hasJellyBeanMr1()) {
					...
					rotation = UtilityAdapter
							.VideoGetMetadataRotate(part.tempPath);
				}
				...
				buffer.append(String.format("%.1f",
						(part.endTime - part.startTime) / 1000F));
				...
				if (!hasAudio) {
					// 生成采样数据 // 采样率44100 * 采样位(unsigned 16bit = 2)* 声道(1) *
					// 时间(1秒)
				...
				boolean result = UtilityAdapter
						.FFmpegRun("", buffer.toString()) == 0;
	...
	// 分离音视频
	public static int DemuxMedia(String path, String[] result) {

		String outPath = VCamera.getVideoCachePath() + "demux/"
				+ System.currentTimeMillis() + "/";
		String outFile = new File(path).getName();
		new File(outPath).mkdirs();

		String cmd1 = String.format(
				"ffmpeg -y -i \"%s\" -an -vcodec copy %s -vn -acodec copy %s",
				path, outPath + outFile, outPath + outFile + ".aac");

		int res1 = UtilityAdapter.FFmpegRun("", cmd1);

		if (res1 != 0) {
			VCamera.copyFFmpegLog(cmd1.toString());
		}

		result[0] = outPath + outFile;
		result[1] = outPath + outFile + ".aac";

		return res1;
	}

	// 合并音视频
	public static int MuxMedia(String videoPath, String musicPath,
			float voiceVolume, float musicVolume, String outPath) {
		String cmd;
		if (musicPath == null || musicPath.isEmpty() || musicVolume <= 0) {
			// 此时没有配乐,只有原音
			cmd = String
					.format("ffmpeg -y -i %s  -filter_complex \"[0:a]volume=%.1f\" -strict -2 -acodec aac -vcodec copy %s",
							videoPath, voiceVolume, outPath);
		} else {

			cmd = String
					.format("ffmpeg -y -i %s -i %s -filter_complex \"[0:a]volume=%.1f [a0];[1:a]volume=%.1f [a1];[a1][a0] amix=inputs=2:duration=first:dropout_transition=2\" -strict -2 -acodec aac -vcodec copy %s",
							musicPath, videoPath, musicVolume, voiceVolume,
							outPath);
		}

		int res1 = UtilityAdapter.FFmpegRun("", cmd);

		if (res1 != 0) {
			VCamera.copyFFmpegLog(cmd.toString());
		}
		...
		if (part != null && !StringUtils.isEmpty(part.tempPath)) {
			...
			if (f != null && f.exists() && !f.isDirectory()) {
				...
				try {
					...
					width = exif.getAttributeInt(ExifInterface.TAG_IMAGE_WIDTH,
							0);
					height = exif.getAttributeInt(
							ExifInterface.TAG_IMAGE_LENGTH, 0);
					rotation = exif.getAttributeInt(
				...
				if (width > 0 && height > 0) {
					...
					case ExifInterface.ORIENTATION_ROTATE_90:
						scaleBuffer
								.append("transpose=1[transpose];[transpose]");
						break;
					case ExifInterface.ORIENTATION_ROTATE_180:
						scaleBuffer
								.append("transpose=2[transpose];[transpose]");
						break;
					case ExifInterface.ORIENTATION_ROTATE_270:
						scaleBuffer
								.append("vflip[vflip];[vflip]hflip[transpose];[transpose]");
						break;
					}
				...
				// -s 480x480
//				String cmd = String
//						.format("ffmpeg %s -y -loop 1 -f image2 -i \"%s\"  -vcodec rawvideo -r 15 -t %.1f -f rawvideo %s -pix_fmt yuv420p \"%s\"",
//								FFMpegUtils.getLogCommand(), part.tempPath,
//								part.duration / 1000F, scaleBuffer.toString(),
//								part.mediaPath);
				
				String cmd = String
						.format("ffmpeg  -y -loop 1 -f image2 -i \"%s\"  -vcodec rawvideo -r %.1f -t %.1f -f rawvideo %s -pix_fmt yuv420p \"%s\"",
								 part.tempPath,
								1000f/part.duration,
								part.duration / 1000F, scaleBuffer.toString(),
								part.mediaPath);
...
//				String cmd = String
//						.format("ffmpeg %s -y -loop 1 -f image2 -i \"%s\"  -vcodec h264 -r %.1f -t %.1f   %s  \"%s\"",
//								FFMpegUtils.getLogCommand(), part.tempPath,
//								1000F/part.duration,
//								part.duration / 1000F, scaleBuffer.toString(),
//								part.mediaPath);
				
				if (UtilityAdapter.FFmpegRun("", cmd) == 0&&part.audioPath!=null) {
					// === 生成1秒的音频 === //44100*2*1*1
...
//					return true;
	 ...
	 * @return
	 */
	public static boolean captureThumbnails(String videoPath,
			String outputPath, String wh, String ss) {
		// ffmpeg -i /storage/emulated/0/DCIM/04.04.mp4 -s 84x84 -vframes 1
		// /storage/emulated/0/DCIM/Camera/miaopai/1388843007381.jpg
		...
		if (ss == null)
			...
			ss = " -ss " + ss;
		String cmd = String
				.format("ffmpeg -d stdout -loglevel verbose -i \"%s\"%s -s %s -vframes 1 \"%s\"",
						videoPath, ss, wh, outputPath);
	...
	// * @return
	// */
	// public static boolean videoTranscoding(MediaObject mMediaObject, String
	// targetPath, int windowWidth, boolean complexWatermark) {
	// if (mMediaObject == null || StringUtils.isEmpty(targetPath) ||
	// windowWidth <= 0)
	// return false;
	//
	// StringBuffer buffer = new StringBuffer("ffmpeg");
	...
	// float volume = 0.5f ;//theme.volumn;
	// //主题音乐
	// buffer.append(" -i \"");
	// buffer.append(theme.mSoundText);
	// buffer.append("\"");
	...
	//
	// //是否动态水印
	// final int watermarkCount =3;//= theme == null ? 0 :
	// theme.watermarkes.size();
	...
	// widgetsInput.add(String.format(" -i \"%s\"",
	// "watermark"+i/*theme.watermarkes.get(i)*/));
	// // Log.e("FFMpegUtils", "videoTranscoding...frameDuration:" +
	// theme.frameDuration);
	// if (watermarkCount == 1)
	// widgetsOverlay.add("overlay=0:0");
	// else
	// widgetsOverlay.add(String.format("overlay=x='if(eq(floor(mod(t*%d,%d)),%d), 0, -500)':y=0",
	// 1000 / (50 / watermarkCount), watermarkCount, i));
	// }
	// }
	//
	// // //添加小部件
	...
	// // float maxTime = (part.endTime - (part.endTime - part.startTime -
	// part.cutEndTime)) / 1000F;
	// //
	// widgetsOverlay.add(String.format("overlay=x='if(between(t, %.3f, %.3f), %d, -500)':y=%d",
	// minTime, maxTime, (int) (widget.overlayX / videoAspectRatio), (int)
	// (widget.overlayY / videoAspectRatio)));
	// // }
	// // }
	// // }
	//
	...
	// buffer.append(" -b:v " + mMediaObject.getVideoBitrate() +
	// "k -g 30");//-ar 44100 -ac 2 -b:a 64k
