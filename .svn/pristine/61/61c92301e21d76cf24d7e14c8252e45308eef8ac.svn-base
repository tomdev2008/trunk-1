...
public class SwipeCardsView extends LinearLayout {
    ...
    public int getmShowingIndex() {
        return mShowingIndex;
    }
    ...
    private final int SCROLL_DURATION = 400; // scroll back duration
    ...
    private int cardVisibleCount = 4;
    /**
    ...
    private boolean mNoChance = false;
...
//        cardVisibleCount = mAdapter.getVisibleCardCount();
        int visibleCount = Math.min(cardVisibleCount, mCount);
        for (int i = mShowingIndex; i < mShowingIndex + visibleCount; i++) {
            View childView = null;
            int viewPos = i - mShowingIndex;
            if (viewList.size() > 0 && viewList.size() > viewPos) {
                childView = viewList.get(i - mShowingIndex);
            }
            if (childView == null) {
                childView = LayoutInflater.from(getContext()).inflate(getCardLayoutId(mAdapter.getCardLayoutId()), this, false);
                if (childView == null) {
                    return;
                }
...
//        cardVisibleCount = mAdapter.getVisibleCardCount();
        ...
        for (int i = mShowingIndex; i < mShowingIndex + visibleCount; i++) {
            ...
            if (i < mCount) {
                if (i == mShowingIndex + visibleCount - 1) {
                    final int finalI = i;
                    final View finalChildView = childView;
                    postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            bindCardData(finalI, finalChildView);
                        }
                    }, 10);
                } else {
                    bindCardData(i, childView);
...
//            @Override
//            public void run() {
        View changedView = getTopView();
        if (changedView == null) {
            return;
        }
        moveTopView(getMeasuredWidth(), getMeasuredHeight());
...
//        }
    ...
    private void moveTopView(int deltaX, int deltaY) {
        ...
        if (topView != null) {
            ...
            if (mAdapter != null) {
                mAdapter.onTopViewMoved(topView, deltaX, deltaY);
            }
        }
    }
...
//        float alpha = 1.0f * (100 - alphaOffsetStep * index) / 100;
        float alpha = 1.0f - index / ((cardVisibleCount - 1) * 1.0f);
    ...
    private void resetViewGroup() {
        if (releasedViewList.size() == 0) {
            ...
            mAdapter.onTopViewReuse(changedView);
            mScrolling = false;
...
//            addView(changedView,0);
    ...
    private void onTopViewMoved(int dx, int dy) {
...
//        float initAlpha = 1.0f * (100 - alphaOffsetStep * index) / 100;
        float initAlpha = 1.0f - index / ((cardVisibleCount - 1) * 1.0f);
...
//        float nextAlpha = 1.0f * (100 - alphaOffsetStep * (index - 1)) / 100;
        float nextAlpha = 1.0f - (index - 1) / ((cardVisibleCount - 1) * 1.0f);
